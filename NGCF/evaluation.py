...
class Evaluation():
    ...
    def get_metric(self):
        ...
        self.model.eval()
        with torch.no_grad():
            for users,items,label in self.dataloader:
                ...
                #                                              use_dropout=False)
                # RecDCL_BPR eval:
                # user_embeddings, item_embeddings,_ ,_,_,_= self.model(users=users,
                #                                              pos_items=items,
                #                                              neg_items=[],
                #                                              use_dropout=False)
                # RecDCL eval:
                user_embeddings, item_embeddings,_ ,_,_= self.model(users=users,
                                                             pos_items=items,
                                                             neg_items=[],
                                                             use_dropout=False)
                ...
                recommends = torch.take(items, pred_indices).cpu().numpy().tolist()
                labels = torch.take(label,pred_indices).cpu().numpy().tolist()
                N_positive_sample=0
                for item in labels:
                    if item==1:
                        N_positive_sample+=1
                ...
                # 真实正例样本中被预测正确的占比
                Recall.append(N_positive_sample/self.top_k)

...
class NGCF(nn.Module):
    def  __init__(self,
        ...
        self.u_target_his = torch.randn((self.num_users, self.embed_size), requires_grad=False).to(device)
        self.i_target_his = torch.randn((self.num_items, self.embed_size), requires_grad=False).to(device)
    ...
    #     prev_embedding= torch.cat((self.embedding_user.weight,
    #                                 self.embedding_item.weight),dim=0)
    #     embeddings_list=[prev_embedding]
    #     for layer_idx in range(3):
    #         prev_embedding=torch.sparse.mm(norm_laplacian,prev_embedding)
    #         embeddings_list.append(prev_embedding)
    #     lightgcn_all_embeddings=torch.stack(embeddings_list,dim=1)
    #     lightgcn_all_embeddings=torch.mean(lightgcn_all_embeddings,dim=1)
    #     users_embed=lightgcn_all_embeddings[users,:]
    #     pos_item_embeddings = lightgcn_all_embeddings[pos_items, :]
    #     neg_item_embeddings = lightgcn_all_embeddings[neg_items, :]
    ...
    def forward(self,users, pos_items, neg_items,use_dropout):
        ...
        with torch.no_grad():
            u_target, i_target = self.u_target_his.clone()[users, :], self.i_target_his.clone()[pos_items, :]
            u_target.detach()
            i_target.detach()

            # momentum=0.1
            u_target = u_target * 0.1 + users_embed.data * (1. - 0.1)
            i_target = i_target * 0.1 + pos_item_embeddings.data * (1. - 0.1)

            self.u_target_his[users, :] = users_embed.clone()
            self.i_target_his[pos_items, :] = pos_item_embeddings.clone()
        
        return users_embed, pos_item_embeddings, lightgcn_all_embeddings, u_target, i_target
    ...
    #     return users_embed, pos_item_embeddings, lightgcn_all_embeddings, u_target, i_target,neg_item_embeddings

...
# 输出需要数据集
print(f'file category:{args.file_category}')
...
# get number of unique userID, unique  itemId
# 得到userId的个数和itemId的个数
max_num_users,max_num_items = total_dataframe['userId'].max()+1, total_dataframe['itemId'].max()+1
print('data loaded!')

# dataloader for train_dataset
dataloader_train= DataLoader(dataset=train_set,batch_size=args.batch,shuffle=True,num_workers=0,)
...
# 选择需要训练哪种模型
if args.model=='MLP':
    model = MLP(num_users=max_num_users,num_items=max_num_items,num_factor=args.factor,layer=args.layer,use_pretrain=use_pretrain,use_NeuMF=False)
    optimizer = optim.Adam(model.parameters(), lr=args.lr)

elif args.model=='GMF':
    model = GMF(num_users=args.batch*max_num_users,num_items=args.batch*max_num_items,num_factor=args.factor,use_pretrain=use_pretrain,use_NeuMF=False)
    optimizer = optim.Adam(model.parameters(), lr=args.lr)

elif args.model=='NeuMF':
    if use_pretrain:
        GMF_dir = os.path.join(pretrain_dir,'1m_GMF.pth')
        MLP_dir = os.path.join(pretrain_dir,'1m_MLP.pth')
    ...
    model = NeuMF(num_users=max_num_users,
...
if __name__=='__main__' :
    # 初始化需要训练的模型
    train = Train(model=model,optimizer=optimizer,criterion=criterion,epochs=args.epoch,test_obj=dataloader_test,dataloader=dataloader_train,device=device,print_cost=True,)
    ...
    train.train()
    if save_model:
        pretrain_model_dir = os.path.join(pretrain_dir,args.file_category+'_'+args.model+'.pth')
        torch.save(model,pretrain_model_dir)

    end = time.time()
    print(f'training time:{end-start:.5f}')
    # 对模型进行评价
    HR,NDCG,Recall = metrics(model,test_loader=dataloader_test,top_k=args.topk,device=device)
    print("HR: {:.3f}\tNDCG: {:.3f}\tRecall:{:.3f}".format(np.mean(HR), np.mean(NDCG),np.mean(Recall)))

...
class MLP(nn.Module):
    def __init__(self,
        ...
        # print(type(layer[0]))
        # print(layer[0])
        # 对每个userId以及itemId赋予32个特征
        self.user_embedding = nn.Embedding(num_users,int(layer[0])//2)
        self.item_embedding = nn.Embedding(num_items,int(layer[0])//2)
        self.use_NeuMF = use_NeuMF
        MLP_layers=[]
        # 初始化线性层，遍历layer的列表
        for idx,factor in enumerate(layer):
            # 最终为3个线性层(64,32),(32,16),(16,8),每个线性层后面都接一个ReLU激活函数
